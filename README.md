...
2.  **Ключові концепції та архітектура**
    ...
    2.3. Внутрішні модулі та сервіси
        * 2.3.1. Парсер контенту `Mindustry`
        * 2.3.2. Емулятор `Mindustry` логіки
3.  **Функціональні вимоги: Система логування**
    3.1. Технічні логи
        * 3.1.1. Призначення та формат
        * 3.1.2. Перелік подій для логування
    3.2. Канали для Адміністрації та Модерації
        * 3.2.1. Канал `#дії-модераторів`
        * 3.2.2. Канал `#стан-бота`
        * 3.2.3. Канал `#підозри-ші` та логіка кнопки `[Помилкове спрацювання]`
...
4.  **Функціональні вимоги: Модуль AI-Модератор "Вартовий"**
    ...
    4.2. Логіка аналізу та прийняття рішень
        * 4.2.1. Побудова запиту до Gemini та системний промпт
        * 4.2.2. Архітектура довгострокової пам'яті (RAG)
        * 4.2.3. Ескалація покарань
...
5.  **Функціональні вимоги: Детальний опис команд та їх логіка**
    ...
    5.4. Команди, специфічні для `Mindustry`
        * `/опублікувати`
        * `/верифікація-розробника`
        * `/випадковий`
        * `/додати-локалізації`
        * `/знайти-ресурс`
        * `/симуляція-коду`
    5.5. Інструменти для Адміністрації
        * `/опублікувати-новину`
        * `/переіндексувати-пам'ять`
...
8.  **План розробки версії 2.0**
    * Віха 1: Основа та інфраструктура
    * Віха 2: Модуль модерації
    * Віха 3: Модулі спільноти та `Mindustry`
    * Віха 4: Інтеграція AI-Модератора (MVP)
    * Віха 5: MVP веб-редактора новин
    * Віха 6: Тестування та впровадження
...
*   **Команда `/зняти-попередження`**
    ...
    *   **Логіка виконання:**
        ...
        6.  Після вибору бот видаляє відповідний запис з бази даних (`#технічний-лог`) та логує дію.
...
*   **Команда `/очистити-історію`**
    ...
    *   **Логіка виконання:**
        ...
        4. Після вибору бот видаляє відповідний запис з бази даних (`#технічний-лог`) та логує дію.
...
#### **6.3. Тон голосу (Tone of Voice)**
Бот повинен дотримуватися професійного тону комунікації, із моливими легкими та ледь помітним жартами/підколами.
