# **Технічне завдання: Discord-бот "Message" v2.0**
...
*   **Формат запису:** `Logfmt`. Кожна подія записується як один рядок тексту, що містить пари `ключ=значення`.
    ...
    *   **Обробка значень:** Всі текстові значення, що можуть містити пробіли, лапки або інші спеціальні символи, **обов'язково** беруться в подвійні лапки. Внутрішні подвійні лапки екрануються за допомогою `\` (напр., `reason="Користувач сказав \"це погане слово\""`).
*   **Події для логування:**
...
*   **Призначення:** Надання команді сервера зручних, агрегованих звітів для оперативного реагування та моніторингу.
*   **Канал `#дії-модераторів`:**
    *   **Зміст:** Єдина стрічка всіх дій модерації (попередження, бани, м'юти, зняття покарань), виконаних як модераторами вручну, так і автоматично ШІ-модулем. Повідомлення форматуються у вигляді стандартизованих Embed'ів.
*   **Канал `#стан-бота`:**
    *   **Зміст:** Повідомлення про технічний стан бота: успішний старт, перезапуск, помилки у логіці, підключенні до зовнішніх API (Discord, Gemini, GitHub), сповіщення про досягнення лімітів API.
*   **Канал `#підозри-ші`:**
    *   **Зміст:** Детальні звіти від ШІ про неоднозначні потенційні порушення. Кожен звіт містить цитату підозрілого контенту, ID користувача, вердикт ШІ (`violated_rules`, `confidence`, `severity`, `reasoning`) та кнопки для швидких дій (`[Видати попередження], [Проігнорувати], [Помилкове спрацювання]`).
...
*   **Призначення:** Візуальне представлення відфільтрованих подій для зручного моніторингу конкретних аспектів життя сервера.
*   **Канал `#логи-видалення`:**
    *   **Зміст:** Для кожного видаленого повідомлення створюється Embed, що містить автора, канал та повний текст повідомлення. Якщо були вкладення, бот намагається завантажити їх і прикріпити до логу.
*   **Канал `#логи-редакції`:**
    *   **Зміст:** Для кожного відредагованого повідомлення створюється Embed, що містить автора, канал, текст повідомлення "до" та "після", а також пряме посилання на відредаговане повідомлення.
*   **Канал `#логи-учасники`:**
    *   **Зміст:** Embed'и про приєднання/вихід учасників, зміну у їхніх профілях або ролях.
*   **Канал `#звіти-войс`:**
    *   **Зміст:** Фінальні звіти по завершенню голосових сесій. Звіт містить назву каналу, тривалість сесії, список учасників, короткий самарі розмови від ШІ та, у випадку виявлених порушень, прикріплені короткі аудіофайли-докази.
...
Модуль ШІ підписаний на події від Системи Логування та аналізує наступні типи даних:
...
1.  **Побудова запиту до Gemini:** Для кожної події бот формує деталізований запит, що містить:
    *   **Системний промпт (інструкція):**
        ```markdown
        "Ти — пильний модератор української ігрової спільноти Mindustry. Проаналізуй надані дані на порушення наступних правил: [перелік правил сервера]. 
        Формуй своє бачення на основі наданого контексту: [контекст] У відповідь надай лише JSON-об'єкт з точною структурою: 
        { "is_violation": boolean, "violated_rules": ["r1", "r3"], "confidence_score": число від 0 до 100, "severity_score": число від 0 до 100, "reasoning": "коротке пояснення твого рішення українською мовою" }".
        ```
...
*   **Загальний доступ:** Команди доступні виключно для ролей чи користувачів, що мають відповідні права згідно з п. 5.1.
...
*   **Команда `/зняти-попередження`**
    ...
    *   **Логіка виконання:**
        ...
        4.  Відповідь містить кнопки `[Так, зняти попередження]` та `[Скасувати]`.
        5.  Дія виконується лише після натискання на кнопку підтвердження.
        6.  Після вибору бот створює новий запис у `#технічний-лог` з подією `warning_revoked`, що посилається на оригінальне попередження.
    *   **Примітка:** Ця дія не видаляє оригінальний запис про попередження з логу, а створює новий, що його анулює. Це забезпечує цілісність історії.

*   **Команда `/очистити-історію`**
    ...
    *   **Логіка виконання:**
        ...
        4.  Після вибору бот створює новий запис у `#технічний-лог` з подією `history_cleared`, що посилається на користувача.
    *   **Примітка:** Аналогічно до зняття попередження, ця команда не видаляє старі записи, а створює новий, що позначає всі попередні попередження як неактивні.

*   **Команда `/історія-покарань`**
    ...
    *   **Логіка виконання:**
        ...
        2.  Якщо викликається без аргументів, показує користувачеві його власну історію (доступно усім користувачам).
        3.  Якщо кількість записів перевищує 5, відповідь автоматично форматується з пагінацією (кнопками для перегляду сторінок).

*   **Команда `/очистити-чат`**
...
*   **Команда `/інфо`**
    ...
    *   **Логіка виконання:**
        1.  Якщо викликається лише з аргументом `тип`, бот надсилає приватне повідомлення з випадаючим списком доступних розділів (у випадку ЧаПів).
...
*   **Команда `/загуглити`**
    ...
    *   **Логіка виконання:** Виконує пошук із застосуванням фільтру "Безпечний пошук" (SafeSearch) і повертає один, найрелевантніший результат. Відповідь оформлюється у жартівливому стилі (https://lmgt.org).
...
*   **Команда `/опублікувати`**
    ...
    *   **Логіка виконання:**
        1.  Бот парсить наданий файл/текст за допомогою внутрішнього парсера, визначає тип схема чи мапа і самостійно обирає відповідний **форум**, витягує назву та опис із наданого файлу/тексту.
...
*   **Команда `/верифікація-розробника`**
...
*   **Команда `/опублікувати-новину`**
    *   **Загальний доступ:** Команди доступні лише для ролей/учасників з відповідними правами.
    *   **Призначення:** Створення кастомізованих повідомлень у каналі новин.
    *   **Логіка виконання:**
        1.  Бот надсилає приватне повідомлення з унікальним, тимчасовим посиланням на зовнішній веб-редактор.
        2.  Адміністратор автентифікується через Discord (OAuth2).
        3.  Створює повідомлення у візуальному редакторі з живим попереднім переглядом.
...
*   **Команда `/переіндексувати-пам'ять`**
    *   **Загальний доступ:** Команди доступні лише для ролей/учасників з правами адміністратора.
...
#### **6.1. Загальний принцип**
Цей розділ слугує для визначення візуальної ідентичності та тону комунікації бота. Дотримання цих настанов є обов'язковим для забезпечення консистентного та професійного користувацького досвіду.

#### **6.2. Першоджерело стилю**
Єдиним та вичерпним джерелом правди для візуального стилю є набір концепт-вебхуків, наданих Замовником/Розробником. Розробка повинна суворо відтворювати формат, структуру, іконки, кольори та загальний вигляд Embed-повідомлень, представлених у цих концептах. Будь-які візуальні елементи, не визначені в наданих матеріалах, повинні бути розроблені в аналогічному стилі та додатково узгоджені.

#### **6.3. Тон голосу (Tone of Voice)**
Бот повинен дотримуватися професійного тону комунікації, із **можливими** легкими та ледь **помітними** жартами/підколами.
...
*   **Перелік параметрів для конфігурації:**
    ...
    *   **Параметри ШІ:** Поріг впевненості, поріг серйозності, кулдауни для "м'яких" нагадувань.
...
Розробка буде вестися поетапно, за визначеними віхами (milestones).

*   **Віха 1: Основа та інфраструктура (Sprint 1)**
    *   **Задачі:**
        1.  Створення структури проєкту, налаштування середовища розробки.
        2.  Підключення до Discord API, реалізація базового клієнта, що може реагувати на події.
        3.  Створення обробника слеш-команд, що дозволяє реєструвати та виконувати прості команди.
        4.  Реалізація повної системи логування: створення та запис у `#технічний-лог` у форматі `logfmt`, а також у всі тематичні та адміністративні канали (`#логи-видалення`, `#дії-модераторів` і т.д.).
        5.  Створення механізму завантаження конфігурації з файлу.
...
*   **Віха 2: Модуль модерації (Sprint 2)**
    *   **Задачі:**
        1.  Реалізація повного функціоналу всіх команд з розділу 5.2.
        2.  Налаштування системи видачі ролей та прав доступу до команд.
        3.  Налаштування інтерактивних елементів (кнопки підтвердження, випадаючі списки, пагінація).
...
*   **Віха 3: Модулі спільноти та `Mindustry` (Sprint 3-4)**
    *   **Задачі:**
        1.  Реалізація всіх команд з розділів 5.3 та 5.4.
        2.  Інтеграція з API GitHub для команд `/верифікація-розробника` та `/знайти-ресурс`.
        3.  Розробка внутрішніх модулів: Парсер контенту `Mindustry` та Емулятор логіки `Mindustry`.
    *   **Критерії завершення:** Усі команди працюють згідно з ТЗ, взаємодія з GitHub API стабільна.

*   **Віха 4: Інтеграція AI-Модератора (MVP) (Sprint 5)**
    *   **Задачі:**
        1.  Підключення до API Google Gemini.
        2.  Реалізація фонового аналізу тексту та зображень.
        3.  Впровадження логіки прийняття рішень.
...
*   **Віха 5: MVP веб-редактора новин (Sprint 6)**
    *   **Задачі:**
        1.  Розгортання базової інфраструктури для веб-додатку (Frontend/Backend).
        2.  Реалізація системи автентифікації через Discord OAuth2.
        3.  Створення інтерфейсу редактора з функціоналом, визначеним в MVP (створення одного Embed'а, додавання полів, зображення, простого тексту та прикріплених файлів).
        4.  Налаштування взаємодії між веб-додатком та ботом для публікації повідомлень.
    *   **Критерії завершення:** Користувач із відповідними правами може створити та опублікувати новину через веб-інтерфейс згідно з функціоналом MVP.

*   **Віха 6: Тестування та впровадження (Sprint 7)**
    *   **Задачі:**
        1.  Проведення комплексного тестування всіх модулів та їх взаємодії.
        2.  Виправлення виявлених помилок та оптимізація продуктивності.
        3.  Підготовка інструкцій для адміністраторів щодо налаштування та використання бота.
        4.  Фінальне розгортання бота на робочому сервері.
